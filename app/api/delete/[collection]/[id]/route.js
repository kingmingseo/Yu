...
export async function DELETE(request, { params }) {
  ...
  try {
    ...
    // mainImage URL 추가
    if (
      documentToDelete.mainImage &&
      documentToDelete.mainImage.includes("amazonaws.com")
    ) {
    ...
    if (
      documentToDelete.contentImages &&
      Array.isArray(documentToDelete.contentImages)
    ) {
    ...
    if (
      documentToDelete.video &&
      documentToDelete.video.includes("amazonaws.com")
    ) {
    ...
    if (s3UrlsToDelete.length > 0) {
      console.log(
        `S3에서 ${s3UrlsToDelete.length}개 파일 삭제 시작:`,
        s3UrlsToDelete
      );
